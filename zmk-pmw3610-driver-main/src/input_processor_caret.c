...
static int caret_handle_event(const struct device *dev, struct input_event *event, uint32_t param1,
    ...
    /*
     * Гасим движение курсора: в режиме каретки прокат шара целиком уходит в
     * стрелки.
     *
     * Обнуление значения, а не только возврат STOP, — обход поведения ZMK.
     * В input_listener.c, filter_with_input_config():
     *
     *     if (!override->process_next) {
     *         return 0;          // ZMK_INPUT_PROC_CONTINUE, а не ret
     *     }
     *
     * То есть для ПОСЛОЙНОГО переопределения (а мы именно такое) возврат
     * процессора отбрасывается, и событие всё равно доходит до
     * handle_rel_code(). Без обнуления курсор ехал бы одновременно со
     * стрелками. STOP ниже оставлен на случай, если процессор когда-нибудь
     * повесят в базовую цепочку — там возврат учитывается.
     */
    event->value = 0;
